...
// ═══════════════════════════════════════
const KEYS = {
  nombre:'cfg_n', telefono:'cfg_tel', cn:'cfg_cn', ct:'cfg_ct',
...
function handleFileUpload(key, input) {
  ...
  reader.onload = function(e) {
    compressImage(e.target.result, 800, function(compressedB64) {
      const keyMap = { usuario:'imgUsuario', ubicacion:'imgUbicacion', contacto:'imgContacto', meds:'imgMeds', pet:'imgPet' };
      const ck = keyMap[key];
      if (!ck) return;
      C[ck] = compressedB64;
      try {
        localStorage.setItem(KEYS[ck], compressedB64);
      } catch(err) {
        console.error("Storage quota exceeded", err);
      }
      setFilePreview(key, compressedB64);
      applyC();
    });
  };
